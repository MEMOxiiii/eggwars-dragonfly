...
// MoveToWorld moves the player to the target world at the specified position.
// It removes the player from its current world and adds it to the target world,
// notifying the client so the dimension change is applied.
func (p *Player) MoveToWorld(w *world.World, pos mgl64.Vec3) {
	// Remove the player from the current world and obtain its handle.
	handle := p.tx.RemoveEntity(p)

	// Add the handle to the target world in a transaction.
	w.Exec(func(tx *world.Tx) {
		np := tx.AddEntity(handle).(*Player)
		np.Teleport(pos)
		// Tell the session to respawn the controllable client-side so the client
		// recognises the world change.
		np.session().SendRespawn(pos, p)
		np.SetVisible()
	})
}
